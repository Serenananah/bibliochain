...
original_author="Serenananah"
original_name="bibliochain"
original_urlname="bibliochain"
original_description="Awesome bibliochain created by Serenananah" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/bibliochain "${name}"
...
then
    ...
    echo "then run 'bibliochain' or 'python -m bibliochain'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'bibliochain' or 'python -m bibliochain' or 'poetry run bibliochain'"
fi
