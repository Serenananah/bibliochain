...
- 🎯 Entry points to execute your program using `python -m <bibliochain>` or `$ bibliochain` with basic CLI argument parsing.
...
---
# bibliochain Flask Application

Awesome bibliochain created by Serenananah
...
```bash
git clone https://github.com/Serenananah/bibliochain bibliochain
cd bibliochain
make install
```
...
```bash
bibliochain create-db   # run once
bibliochain populate-db  # run once (optional)
bibliochain add-user -u admin -p 1234  # ads a user
bibliochain run
```
